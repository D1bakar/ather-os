...
- **First schedule into init:** `sched::start()` now context-switches to the first runnable task after idle (worker → init) instead of waiting for a timer tick while idle HLTs; syscall buffer copies temporarily switch to the user CR3 so ring-3 `write` reads the init message buffer.
- **Timer preempt disabled for kernel tasks (M6.1):** `tick_preempt()` no longer calls `switch_context` from the timer IRQ stub (partial save corrupts RIP); boot selects the init user task directly with IF clear until `IRETQ` enables interrupts in ring 3.
